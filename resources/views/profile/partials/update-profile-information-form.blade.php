    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-[#16151A] dark:text-white">
                        {{ __('Seu endereço de email não foi verificado.') }}

                        <button form="send-verification" class="underline text-sm text-black/60 dark:text-white/60 hover:text-[#16151A] dark:hover:text-white rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#1B7A43] dark:focus:ring-offset-[#1E1D24]">
